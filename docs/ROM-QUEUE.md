...
| trecision | `Nightlong.zip` (already in `/roms`) | CDRomance release | **Confirmed 2026-09-07**: loads and plays with sound. The caveat was real but release-specific — the archive.org `DreamCatcher_Nightlong_Win95_1998_Eng` release has only 2 of 3 CD-animation files, this one has all three (`NLANIM.CD1/2/3`). Matches the clean `ADGF_NO_FLAGS` English entry (`DATA.NL` 436228 / `7665db13ad2a1ceb576531be3e1efb30`). 1.46 GiB packed, ~3.0 GiB packed+unpacked — inside both EJS ceilings |
...
| grim (`monkey4`) | `Escape from Monkey Island (CD Windows).zip` | repacked from 2 CD ISOs | **One file short.** Detects and loads the engine correctly (`artAll.m4b` 18515664 matches on size and 5000-byte MD5); shows the ADGF_UNSTABLE warning, then fails data verification: *"Could not open the file MonkeyUpdate.exe for checking"*. `EMIEngine::getUpdateFilename()` (`engines/grim/emi/emi.cpp:52`) requires `MonkeyUpdate.exe` for Windows non-demo builds — LucasArts' official 1.1 patch, which is **not on either CD** (checked both ISOs). Needs that patch added, or a dump that includes it. Note every `monkey4` entry is `ADGF_UNSTABLE` |
| grim (`monkey4`) | `efmi_large_demo.sea` | scummvm.org/demos | **Undetectable — no ScummVM signature exists.** The Macintosh demo, listed on scummvm.org/demos with target `grim:monkey4`. Extracts cleanly with `unar` (Compact Pro format), but **0 of its 43 files** match any entry in `engines/grim/detection_tables.h` — verified against current ScummVM master, not just our base. All five `monkey4` demo entries require `magdemo.lab` (19826116) on **Windows**; there are no `kPlatformMacintosh` demo entries. Reported upstream. **The Windows demo (`magdemo.lab`) is the one that would work** |
