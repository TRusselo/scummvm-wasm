...
## Confirmed working so far (42 of 102, + agos2 subengine bonus)
...
| director | The Journeyman Project | **Detection confirmed, boot not yet verified** — sourced the DOS/Windows retail release (archive.org `the-journeyman-project-pc-1.0`, ISO, 454MB extracted, under the size limit), root-level `JMAN.EXE` size- and 5000-byte-hash-matched the `WINGAME1("jman", ...)` entry in `engines/director/detection_tables.h` exactly. Packaged flat with `JMAN.EXE` as the first zip entry; ScummVM correctly identified it and presented its real `ADGF_UNSTABLE` "not yet fully supported... Start anyway?" warning dialog — solid proof the packaging and detection are both correct. Could not get past that dialog to confirm actual gameplay this session: ROMM's frontend (`"VERSION":"development"` per `/api/heartbeat`) was serving different JS chunk hashes on successive page loads during testing (`activity-Cl5zpX1M.js` vs `activity-BIhB0-_E.js` for the same logical chunk, etc.), and every subsequent click on "Start anyway"/"Play" produced zero network requests (confirmed via network-request monitoring) alongside recurring `InvalidStateError: Transition was aborted because of invalid state` and backend HTTP 500s in the console — a live dev-server rebuild/instability, not a ScummVM or packaging issue. Retried across two separate browser tabs with hard reloads between attempts; same signature each time. Needs a retest once the ROMM instance is stable | Yes | archive.org (`the-journeyman-project-pc-1.0`) |
