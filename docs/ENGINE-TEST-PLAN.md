...
## Confirmed working so far (48 of 102, + agos2 subengine bonus)
...
`pegasus`. Marked **Confirmed working** in their table row below — search this file for that phrase to
jump to them.
...
| mm | World of Xeen (via `xeen` subengine — newest/most feature-rich of mm1/xeen; base `mm` has no standalone game) | **Packaged, awaiting user test** — used the pre-extracted archive.org item `msdos_Might_and_Magic_45_-_World_of_Xeen_1994` (33MB total, `MM6/` folder) instead of hunting down Clouds of Xeen alone, since the standalone floppy-disk release turned out to be a multi-volume ARJ archive (`XEEN.A01`-`A05`) that 7z can partially list but not actually extract ("Unsupported Method") — no DOSBox/DEARJ available to run the real DOS installer. The combined World of Xeen package sidesteps that entirely and is tiny regardless. Both `xeen.cc` and `dark.cc` 5000-byte-prefix hash-verified exactly against the `worldofxeen` English DOS entry. Deployed as `[test] Might and Magic - World of Xeen.zip` | Yes | archive.org `msdos_Might_and_Magic_45_-_World_of_Xeen_1994` |
| tsage | Return to Ringworld (via `ringworld2` subengine — sequel, newest of the tsage-based titles; base `tsage` has no standalone game; based on Larry Niven's Hugo/Nebula-winning novel) | **Packaged, awaiting user test** — archive.org's `msdos_Return_to_Ringworld_1994` item bundles both a raw CD image (`rw2/cd/RING2.bin`/`.cue`) and, separately, a pre-built `rw2/scummvm/` folder clearly prepared for ScummVM use — used that folder directly, no CD extraction needed at all. `R2RW.RLB` (47586928 bytes) 5000-byte-prefix hash-matched the English CD detection entry exactly (`df6c25622387007788ca36d99362c1f0`). ~107MB packaged. Deployed as `[test] Return to Ringworld.zip` | Yes | archive.org `msdos_Return_to_Ringworld_1994` |
...
| plumbers | Plumbers Don't Wear Ties | **Packaged, awaiting user test** — archive.org's `Plumbers_Dont_Wear_Ties_PC_Version` item, clean extracted PC data (no DOSBox-wrapper cruft). `GAME.BIN` 5000-byte-prefix hash-verified exactly (`02c965b11e952ce1ee83019576f5aef5`). ~501MB uncompressed. Deployed as `[test] Plumbers Dont Wear Ties.zip` | Yes | archive.org `Plumbers_Dont_Wear_Ties_PC_Version` |
| private | Private Eye | **Packaged, awaiting user test** — archive.org's `private-eye` item (7z, 207MB). `pvteye.z` 5000-byte-prefix hash-matched the EU/UK English detection entry exactly (`d9ce391395701615e8b5d04bc4bf7ec3`, EN_GRB), not the US entry — still English, fine per the "prefer English" rule. ~352MB uncompressed. Deployed as `[test] Private Eye.zip` | Yes | archive.org `private-eye` |
...
| alg | Mad Dog McCree | **Packaged, awaiting user test** — used Crime Patrol v1.00 (DOS) instead: archive.org `msdos_Crime_Patrol_1994` ships as a raw installer ISO inside its zip; extracted just the 6 top-level files needed (`CP.EXE`, `CPDS.LIB`, `CPHELP.EXE`, `CPSB8.EXE`, `INSTALL.EXE`, `README.EXE`, ~291MB total), skipping the separate `CPSS.LIB` (193MB, not part of the detection entry) and the bundled `SPDEMO/` folder. `CPDS.LIB` 5000-byte-prefix MD5 (`43579f72207298f154f6fb2b1a24e193`) and size (303710700 bytes) match the detection entry exactly | Yes | archive.org (`msdos_Crime_Patrol_1994`) |
| avalanche | Lord Avalot d'Argent | **Packaged, awaiting user test** — freeware DOS release (159 files, flat, ~3.1MB), archive.org item `LordAvalotDArgent_1020`. `AVALOT.SEZ` 5000-byte-prefix MD5 (`de10eb353228013da3d3297784f81ff9`) and size (48763 bytes) match the detection entry exactly | Yes | archive.org (`LordAvalotDArgent_1020`) |
| bagel | The Space Bar | **Packaged, awaiting user test** — used Hodj 'n' Podj instead: The Space Bar's only real dumps are 3-CD (~2GB total raw ISOs), well over budget. Hodj 'n' Podj (archive.org `Nova_HodjnPodj_USA`, single-CD `.bin`/`.cue`, converted via `bchunk`) extracted to 2301 files, ~269MB. `META/META.GTL` and `VIDEO/HODJ.AVI` both match the detection entry's 5000-byte-prefix MD5s and exact sizes | Yes | archive.org (`Nova_HodjnPodj_USA`) |
| bbvs | Beavis and Butt-Head in Virtual Stupidity | **Packaged, awaiting user test** — English Windows CD-ROM release, archive.org `CA-WINDOWS-Beavis-and-Butt-Head-in-Virtual-Stupidity` (1300 files, ~411MB). `VNM/VSPR0001.VNM` 5000-byte-prefix MD5 (`7ffe9b9e7ca322db1d48e86f5130578e`) and size (1166628 bytes) match the detection entry exactly | Yes | archive.org (`CA-WINDOWS-Beavis-and-Butt-Head-in-Virtual-Stupidity`) |
...
| chamber | Chamber of the Sci-Mutant Priestess | **Packaged, awaiting user test** — European "Kult" release (DOS, 26 files, flat, ~700KB), archive.org `msdos_Chamber_of_the_Sci-Fi_Mutant_Priestess_1989`. `kult1.pxi` 5000-byte-prefix MD5 (`fc0bd31a3c380338f76ff53e421e47b6`) and size (140537 bytes) match the detection entry exactly | Yes | archive.org (`msdos_Chamber_of_the_Sci-Fi_Mutant_Priestess_1989`) |
